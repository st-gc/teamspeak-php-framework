...
use TeamSpeak\Model\Uri;
use TeamSpeak\Helper\Convert;
use TeamSpeak\Helper\Crypt;
...
class HostNode extends AbstractNode
{
	...
	/**
	 * The HostNode constructor.
	 *
	 * @param  ServerQueryAdapter $squery
	 *
	 * @return HostNode
	 */
	public function __construct( ServerQueryAdapter $squery )
	{
		$this->parent = $squery;
	}
	 ...
	 * @param  integer $sid
	 *
	 * @throws \TeamSpeak\Exception\ServerQueryException
	 * @return integer
	 */
	...
	/**
	 * Returns the ServerNode object matching the currently selected ID.
	 *
	 * @return ServerNode
	 */
	...
	/**
	 * Returns the ServerNode object matching the given ID.
	 *
	 * @param  integer $sid
	 *
	 * @return ServerNode
	...
	public function serverGetById( $sid )
	{
		...
		return new ServerNode( $this, array( "virtualserver_id" => intval( $sid ) ) );
	}

	/**
	 * Returns the ServerNode object matching the given port number.
	 *
	 * @param  integer $port
	 *
	 * @return ServerNode
	...
	public function serverGetByPort( $port )
	{
		...
		return new ServerNode( $this, array( "virtualserver_id" => $this->serverSelectedId() ) );
	}

	/**
	 * Returns the first ServerNode object matching the given name.
	 *
	 * @param  string $name
	 *
	...
	/**
	 * Returns the first ServerNode object matching the given unique identifier.
	 *
	 * @param  string $uid
	 *
	 * @throws ServerQueryException
	...
	public function serverGetByTSDNS( $tsdns )
	{
		...
		while ( $part = array_shift( $parts ) ) {
			...
			try {
				$port = Constant::factory( "tsdns://" . $query . "/?timeout=3" )->resolve( $tsdns )->section(
					":",
					1
				);
	...
	public function serverCreate( array $properties = array() )
	{
		...
		$server = new ServerNode( $this, array( "virtualserver_id" => intval( $detail[ "sid" ] ) ) );
	...
	/**
	 * Returns an array filled with ServerNode objects.
	 *
	 * @param  array $filter
	 *
	 * @return array
	...
	public function serverList( array $filter = array() )
	{
		if ( $this->serverList === null ) {
			...
			foreach ( $servers as $sid => $server ) {
				$this->serverList[ $sid ] = new ServerNode( $this, $server );
			}
	...
	public function permissionList()
	{
		...
		foreach ( $this->permissionList as $permname => $permdata ) {
			...
			$this->permissionList[ $permname ][ "permcatid" ] = $this->permissionGetCategoryById(
				$permdata[ "permid" ]
			);
			...
			if ( !$permdata[ "permname" ]->startsWith(
				...
				$this->permissionList[ $grantsid ][ "permname" ]  = String::factory( $grantsid );
	...
	public function whoamiSet( $ident, $value = null )
	{
		...
		$this->whoami[ $ident ] = ( is_numeric( $value ) ) ? intval( $value ) : String::factory(
			$value
		);
	}
	...
	protected function fetchPermissionList()
	{
		...
		foreach ( $reply as $line ) {
			if ( array_key_exists( "group_id_end", $line ) ) {
				...
				$this->permissionList[ $line[ "permname" ]->toString() ] = array_merge(
					array( "permid" => $start++ ),
					$line
				);
			}
	...
	/**
	 * Returns the underlying ServerQueryAdapter object.
	 *
