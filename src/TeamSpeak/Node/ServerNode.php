...
class ServerNode extends AbstractNode
{
	 ...
	 * @return ServerNode
	 */
	public function __construct( HostNode $host, array $info, $index = "virtualserver_id" )
	{
		$this->parent   = $host;
		$this->nodeInfo = $info;
	...
	public function channelList( array $filter = array() )
	{
		if ( $this->channelList === null ) {
			...
			foreach ( $channels as $cid => $channel ) {
				$this->channelList[ $cid ] = new ChannelNode( $this, $channel );
			}
	...
	/**
	 * Returns TRUE if the given ChannelNode object is a spacer.
	 *
	 * @param  ChannelNode $channel
	 *
	 * @return boolean
	 */
	public function channelIsSpacer( ChannelNode $channel )
	{
		return ( preg_match(
				"/\[[^\]]*spacer[^\]]*\]/",
				$channel
			) && $channel[ "channel_flag_permanent" ] && !$channel[ "pid" ] ) ? true : false;
	}
	 ...
	 *
	 * @throws \TeamSpeak\Exception\Adapter\ServerQueryException
	 * @return integer
	 */
	public function channelSpacerCreate(
		$ident,
		$type = Constant::SPACER_SOLIDLINE,
		$align = Constant::SPACER_ALIGN_REPEAT,
		$order = null,
		$maxClients = 0
	) {
		$properties = array(
			"channel_name_phonetic"                   => "channel spacer",
			"channel_codec"                           => Constant::CODEC_OPUS_VOICE,
		...
		switch ( $type ) {
			case (string)Constant::SPACER_SOLIDLINE:
				$properties[ "channel_name" ] .= "___";
				break;

			case (string)Constant::SPACER_DASHLINE:
				$properties[ "channel_name" ] .= "---";
				break;

			case (string)Constant::SPACER_DOTLINE:
				$properties[ "channel_name" ] .= "...";
				break;

			case (string)Constant::SPACER_DASHDOTLINE:
				$properties[ "channel_name" ] .= "-.-";
				break;

			case (string)Constant::SPACER_DASHDOTDOTLINE:
				$properties[ "channel_name" ] .= "-..";
				break;
	...
	public function channelSpacerGetType( $cid )
	{
		...
		switch ( $channel[ "channel_name" ]->section( "]", 1 ) ) {
			case "___":
				return Constant::SPACER_SOLIDLINE;

			case "---":
				return Constant::SPACER_DASHLINE;

			case "...":
				return Constant::SPACER_DOTLINE;

			case "-.-":
				return Constant::SPACER_DASHDOTLINE;

			case "-..":
				return Constant::SPACER_DASHDOTDOTLINE;

			default:
				return Constant::SPACER_CUSTOM;
		}
	}
	...
	public function channelSpacerGetAlign( $cid )
	{
		...
		if ( !$this->channelIsSpacer( $channel ) || !preg_match(
				"/\[(.*)spacer.*\]/",
				$channel,
				$matches
			) || !isset( $matches[ 1 ] )
		...
		switch ( $matches[ 1 ] ) {
			case "*":
				return Constant::SPACER_ALIGN_REPEAT;

			case "c":
				return Constant::SPACER_ALIGN_CENTER;

			case "r":
				return Constant::SPACER_ALIGN_RIGHT;

			default:
				return Constant::SPACER_ALIGN_LEFT;
		}
	}
	...
	public function channelClientPermAssign( $cid, $cldbid, $permid, $permvalue )
	{
		...
		$this->execute(
			...
			array( "cid" => $cid, "cldbid" => $cldbid, $permident => $permid, "permvalue" => $permvalue )
		);
	}
	...
	public function channelFileList( $cid, $cpw = "", $path = "/", $recursive = false )
	{
		...
		for ( $i = 0; $i < $count; $i++ ) {
			...
			if ( $recursive && $files[ $i ][ "type" ] == Constant::FILE_TYPE_DIRECTORY ) {
				$files = array_merge(
					$files,
					$this->channelFileList( $cid, $cpw, $path . $files[ $i ][ "name" ], $recursive )
				);
			}
		}
	...
	{
		return array_pop(
			$this->execute( "ftgetfileinfo", array( "cid" => $cid, "cpw" => $cpw, "name" => $name ) )->toArray()
		);
	}
	...
	/**
	 * Returns an array filled with ClientNode objects.
	 *
	 * @param  array $filter
	 *
	 * @return array
	...
	public function clientList( array $filter = array() )
	{
		if ( $this->clientList === null ) {
			...
			foreach ( $clients as $clid => $client ) {
				...
				$this->clientList[ $clid ] = new ClientNode( $this, $client );
			}
	...
	{
		return $this->execute( "clientdblist -count", array( "start" => $offset, "duration" => $limit ) )->toAssocArray(
			"cldbid"
		);
	}
	...
	{
		return array_keys(
			$this->execute( "clientdbfind", array( "pattern" => $pattern, ( $uid ) ? "-uid" : null ) )->toAssocArray(
				"cldbid"
			)
		);
	}
	...
	/**
	 * Returns the ClientNode object matching the given ID.
	 *
	 * @param  integer $clid
	 *
	 * @throws ServerQueryException
	...
	/**
	 * Returns the ClientNode object matching the given name.
	 *
	 * @param  string $name
	 *
	 * @throws ServerQueryException
	...
	/**
	 * Returns the ClientNode object matching the given unique identifier.
	 *
	 * @param  string $uid
	 *
	 * @throws ServerQueryException
	...
	/**
	 * Returns the ClientNode object matching the given database ID.
	 *
	 * @param  integer $dbid
	 *
	 * @throws ServerQueryException
	 ...
	 * @return void
	 */
	public function clientKick( $clid, $reasonid = Constant::KICK_CHANNEL, $reasonmsg = null )
	{
		$this->clientListReset();
	...
	public function clientBan( $clid, $timeseconds = null, $reason = null )
	{
		...
		$bans = $this->execute(
			"banclient",
			array( "clid" => $clid, "time" => $timeseconds, "banreason" => $reason )
		)->toAssocArray( "banid" );
	...
	public function clientPermList( $cldbid, $permsid = false )
	{
		...
		return $this->execute(
			"clientpermlist",
			array( "cldbid" => $cldbid, $permsid ? "-permsid" : null )
		)->toAssocArray( $permsid ? "permsid" : "permid" );
	}
	...
	public function clientPermAssign( $cldbid, $permid, $permvalue, $permskip = false )
	{
		...
		$this->execute(
			"clientaddperm",
			array( "cldbid" => $cldbid, $permident => $permid, "permvalue" => $permvalue, "permskip" => $permskip )
		);
	}
	...
	public function serverGroupList( array $filter = array() )
	{
		if ( $this->sgroupList === null ) {
			...
			foreach ( $this->sgroupList as $sgid => $group ) {
				$this->sgroupList[ $sgid ] = new ServerGroupNode( $this, $group );
			}
	 ...
	 * @return integer
	 */
	public function serverGroupCreate( $name, $type = Constant::GROUP_DBTYPE_REGULAR )
	{
	 ...
	 * @return integer
	 */
	public function serverGroupCopy( $ssgid, $name = null, $tsgid = 0, $type = Constant::GROUP_DBTYPE_REGULAR )
	{
		$this->serverGroupListReset();

		$sgid = $this->execute(
			"servergroupcopy",
			array( "ssgid" => $ssgid, "tsgid" => $tsgid, "name" => $name, "type" => $type )
		)->toList();
	...
	/**
	 * Returns the ServerGroupNode object matching the given ID.
	 *
	 * @param  integer $sgid
	 *
	 * @throws ServerQueryException
	...
	/**
	 * Returns the ServerGroupNode object matching the given name.
	 *
	 * @param  string  $name
	 * @param  integer $type
	 *
	 ...
	 * @return ServerGroupNode
	 */
	public function serverGroupGetByName( $name, $type = Constant::GROUP_DBTYPE_REGULAR )
	{
	...
	public function serverGroupPermList( $sgid, $permsid = false )
	{
		return $this->execute(
			"servergrouppermlist",
			array( "sgid" => $sgid, $permsid ? "-permsid" : null )
		)->toAssocArray( $permsid ? "permsid" : "permid" );
	}
	...
	public function serverGroupGetProfiles()
	{
		...
		foreach ( $this->serverGroupList() as $sgid => $sgroup ) {
			if ( $sgroup[ "type" ] != Constant::GROUP_DBTYPE_REGULAR ) {
				continue;
			}
			...
			foreach ( $perms as $permsid => $perm ) {
				...
				} elseif ( String::factory( $permsid )->startsWith( "i_needed_modify_power_" ) ) {
	 ...
	 * @return ServerGroupNode
	 */
	public function serverGroupIdentify( $mode = Constant::GROUP_IDENTIFIY_STRONGEST )
	{
		$profiles = $this->serverGroupGetProfiles();

		$best_guess_profile = ( $mode == Constant::GROUP_IDENTIFIY_STRONGEST ) ? array_shift( $profiles ) : array_pop(
			$profiles
		);
	...
	public function channelGroupList( array $filter = array() )
	{
		if ( $this->cgroupList === null ) {
			...
			foreach ( $this->cgroupList as $cgid => $group ) {
				$this->cgroupList[ $cgid ] = new ChannelGroupNode( $this, $group );
			}
	 ...
	 * @return integer
	 */
	public function channelGroupCreate( $name, $type = Constant::GROUP_DBTYPE_REGULAR )
	{
	 ...
	 * @return integer
	 */
	public function channelGroupCopy( $scgid, $name = null, $tcgid = 0, $type = Constant::GROUP_DBTYPE_REGULAR )
	{
		$this->channelGroupListReset();

		$cgid = $this->execute(
			"channelgroupcopy",
			array( "scgid" => $scgid, "tcgid" => $tcgid, "name" => $name, "type" => $type )
		)->toList();
	...
	/**
	 * Returns the ChannelGroupNode object matching the given ID.
	 *
	 * @param  integer $cgid
	 *
	 * @throws ServerQueryException
	...
	/**
	 * Returns the ChannelGroupNode object matching the given name.
	 *
	 * @param  string  $name
	 * @param  integer $type
	 *
	 ...
	 * @return ChannelGroupNode
	 */
	public function channelGroupGetByName( $name, $type = Constant::GROUP_DBTYPE_REGULAR )
	{
	...
	public function channelGroupPermList( $cgid, $permsid = false )
	{
		return $this->execute(
			...
			array( "cgid" => $cgid, $permsid ? "-permsid" : null )
		)->toAssocArray( $permsid ? "permsid" : "permid" );
	}
	...
	public function channelGroupPermAssign( $cgid, $permid, $permvalue )
	{
		...
		$this->execute(
			"channelgroupaddperm",
			array( "cgid" => $cgid, $permident => $permid, "permvalue" => $permvalue )
		);
	}
	...
	public function channelGroupClientList( $cgid = null, $cid = null, $cldbid = null )
	{
		...
		return $this->execute(
			...
			array( "cgid" => $cgid, "cid" => $cid, "cldbid" => $cldbid )
		)->toArray();
	}
	...
	{
		$download = $this->execute(
			"ftinitdownload",
			array( "clientftfid" => $clientftfid, "cid" => $cid, "name" => $name, "cpw" => $cpw, "seekpos" => $seekpos )
		)->toList();
	...
	public function iconDownload()
	{
		...
		$download = $this->transferInitDownload(
			rand( 0x0000, 0xFFFF ),
			0,
			$this->iconGetName( "virtualserver_icon_id" )
		);
	...
	public function message( $msg )
	{
		$this->execute(
			"sendtextmessage",
			array( "msg" => $msg, "target" => $this->getId(), "targetmode" => Constant::TEXTMSG_SERVER )
		);
	}
	 ...
	 * @return string
	 */
	public function snapshotCreate( $mode = Constant::SNAPSHOT_STRING )
	{
	...
	public function privilegeKeyList( $resolve = false )
	{
		...
		if ( $resolve ) {
			foreach ( $tokens as $token => $array ) {
				...
				try {
					if ( $array[ "token_type" ] ) {
						$tokens[ $token ][ "token_id2" ] = $this->channelGetById(
							$array[ "token_id2" ]
						)->getPathway();
					}
				} catch ( Exception $e ) {
	...
	public function tokenCreate(
		$type = Constant::TOKEN_SERVERGROUP,
		$id1,
		$id2 = 0,
		$description = null,
		$customset = null
	...
	public function tempPasswordCreate( $pw, $duration, $tcid = 0, $tcpw = "", $desc = "" )
	{
		$this->execute(
			...
			array( "pw" => $pw, "duration" => $duration, "tcid" => $tcid, "tcpw" => $tcpw, "desc" => $desc )
		);
	}
	...
	public function logView( $lines = 30, $begin_pos = null, $reverse = null, $instance = null )
	{
		return $this->execute(
			"logview",
			array( "lines" => $lines, "begin_pos" => $begin_pos, "instance" => $instance, "reverse" => $reverse )
		)->toArray();
	}
	 ...
	 * @return void
	 */
	public function logAdd( $logmsg, $loglevel = Constant::LOGLEVEL_INFO )
	{
	...
	public function sendPluginCmd( $plugin, $data )
	{
		$this->execute(
			"plugincmd",
			array( "name" => $plugin, "data" => $data, "targetmode" => Constant::PLUGINCMD_SERVER )
		);
	}
	...
	public function selfPermOverview()
	{
		return $this->execute(
			"permoverview",
			array(
				"cldbid" => $this->whoamiGet( "client_database_id" ),
				"cid"    => $this->whoamiGet( "client_channel_id" ),
				"permid" => 0
			)
		)->toArray();
	}
	 ...
	 * @return integer
	 */
	protected static function sortClientList( ClientNode $a, ClientNode $b )
	{
	...
	protected static function sortGroupList( AbstractNode $a, AbstractNode $b )
	{
		...
		if ( !$a instanceof ServerGroupNode && !$a instanceof ChannelGroupNode ) {
			return 0;
	...
	{
		if ( !array_key_exists( "src", $a ) || !array_key_exists( "src", $b ) || !array_key_exists(
				"type",
				$a
			) || !array_key_exists( "type", $b )
		) {
			return 0;
