...
use TeamSpeak\Node\ServerGroupNode;
use TeamSpeak\Node\ChannelNode;
use TeamSpeak\Node\ChannelGroupNode;
...
class HtmlViewer implements ViewerInterface
{
	 ...
	 *
	 * @return \TeamSpeak\Viewer\HtmlViewer
	 */
	public function __construct( $iconpath = "Image/Viewer/", $flagpath = null, $ftclient = null, $pattern = null )
	{
	...
	protected function getCorpusClass()
	{
		...
		if ( $this->currObj instanceof ChannelNode && $this->currObj->isSpacer() ) {
	...
	{
		if ( $this->currObj instanceof ServerNode ) {
		...
		} elseif ( $this->currObj instanceof ServerGroupNode || $this->currObj instanceof ChannelGroupNode ) {
	...
	{
		if ( $this->currObj instanceof ChannelNode && $this->currObj->isSpacer() ) {
			if ( $this->currObj->spacerGetType() != Constant::SPACER_CUSTOM ) {
				return;
			}

			$string = $this->currObj[ "channel_name" ]->section( "]", 1, 99 );

			if ( $this->currObj->spacerGetAlign() == Constant::SPACER_ALIGN_REPEAT ) {
		...
		if ( $this->currObj instanceof ClientNode ) {
			$before = array();
			$behind = array();

			foreach ( $this->currObj->memberOf() as $group ) {
				if ( $group->getProperty( "namemode" ) == Constant::GROUP_NAMEMODE_BEFORE ) {
					$before[ ] = "[" . htmlspecialchars( $group[ "name" ] ) . "]";
				} elseif ( $group->getProperty( "namemode" ) == Constant::GROUP_NAMEMODE_BEHIND ) {
	...
	protected function getSuffixIcon()
	{
		...
		} elseif ( $this->currObj instanceof ChannelNode ) {
	...
	protected function getSuffixIconChannel()
	{
		...
		if ( $this->currObj[ "channel_codec" ] == Constant::CODEC_CELT_MONO || $this->currObj[ "channel_codec" ] == Constant::CODEC_OPUS_MUSIC ) {
		...
		if ( $this->currObj[ "channel_icon_id" ] ) {
			if ( !$this->currObj->iconIsLocal( "channel_icon_id" ) && $this->ftclient ) {
				...
				$html .= $this->getImage(
					"group_icon_" . $this->currObj[ "channel_icon_id" ] . ".png",
					"Channel Icon"
				);
			}
		}
